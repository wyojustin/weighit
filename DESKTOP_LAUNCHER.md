...
# WeighIt Desktop Launcher Installation
...
This will:
- Create a desktop launcher with the correct paths for your installation
- Install it to your application menu
...
1. Copy the template and replace paths:
   ```bash
   sed "s|INSTALL_DIR|$(pwd)|g" weighit.desktop.template > weighit.desktop
   ```
...
3. Copy to applications directory:
   ```bash
...
If you want to customize the launcher icon or other properties, edit the generated `weighit.desktop` file in `~/.local/share/applications/`.
